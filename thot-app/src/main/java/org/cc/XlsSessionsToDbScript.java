...
public class XlsSessionsToDbScript {
	...
	for (Session session : sessions) {
	    System.out.println("insert into session (id,date,start,end,title,author,author2,description," + "location,type,authorinfo,author2info,authorimgurl,author2imgurl) values ("
		    + session.getId() + ", '" + r(session.getDate()) + "', '" + r(session.getStart()) + "', '" + r(session.getEnd()) + "', '" + r(session.getTitle()) + "', '" + r(session.getAuthor())
		    + "', '" + r(session.getAuthor2()) + "', '" + r(session.getDescription()) + "', '" + r(session.getLocation()) + "', '" + r(session.getType().toString()) + "', '"
		    + r(session.getAuthorInfo()) + "', '" + r(session.getAuthor2Info()) + "', '" + r(session.getAuthorImgUrl()) + "', '" + r(session.getAuthor2ImgUrl()) + "');");
